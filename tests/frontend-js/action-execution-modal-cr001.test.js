/**
 * Tests for CR-001: Append --execution@{mode} flag to CLI command
...
describe('CR-001: --execution@ flag based on auto-proceed', () => {
  ...
  describe('_buildExecutionFlag', () => {
    it('should return --execution@{keep running} for auto mode', () => {
      const Modal = globalThis.ActionExecutionModal;
      const modal = new Modal({ actionKey: 'refine_idea', workflowName: 'hello' });
      modal._autoProceed = 'auto';
      expect(modal._buildExecutionFlag()).toBe(' --execution@{keep running}');
    });

    it('should return --execution@{keep running stop only on question} for stop_for_question', () => {
      const Modal = globalThis.ActionExecutionModal;
      const modal = new Modal({ actionKey: 'refine_idea', workflowName: 'hello' });
      modal._autoProceed = 'stop_for_question';
      expect(modal._buildExecutionFlag()).toBe(' --execution@{keep running stop only on question}');
    });
  ...
  describe('_buildCommand with execution flag', () => {
    it('should include --execution@ flag when auto mode', async () => {
      mockFetchWithAutoProceed('auto');
      const Modal = globalThis.ActionExecutionModal;
      const modal = new Modal({ actionKey: 'refine_idea', workflowName: 'hello' });
      ...
      expect(cmd).toContain('--execution@{keep running}');
    });

    it('should include --execution@ flag when stop_for_question mode', async () => {
      mockFetchWithAutoProceed('stop_for_question');
      const Modal = globalThis.ActionExecutionModal;
      const modal = new Modal({ actionKey: 'refine_idea', workflowName: 'hello' });
      await modal._loadInstructions();
      modal._autoProceed = 'stop_for_question';
      const cmd = modal._buildCommand('');
      expect(cmd).toContain('--execution@{keep running stop only on question}');
    });

    it('should NOT include --execution@ flag when manual mode', async () => {
      ...
      modal._autoProceed = 'manual';
      const cmd = modal._buildCommand('');
      expect(cmd).not.toContain('--execution@');
    });

    it('should place --execution@ before --workflow-mode', async () => {
      mockFetchWithAutoProceed('auto');
      const Modal = globalThis.ActionExecutionModal;
      const modal = new Modal({ actionKey: 'refine_idea', workflowName: 'hello' });
      await modal._loadInstructions();
      modal._autoProceed = 'auto';
      const cmd = modal._buildCommand('');
      const execIdx = cmd.indexOf('--execution@');
      const wfIdx = cmd.indexOf('--workflow-mode');
      expect(execIdx).toBeGreaterThan(-1);
      expect(wfIdx).toBeGreaterThan(execIdx);
    });
  });

  describe('_handleExecute with execution flag', () => {
    it('should send command with --execution@ flag to terminal', async () => {
      mockFetchWithAutoProceed('auto');
      const Modal = globalThis.ActionExecutionModal;
      const modal = new Modal({ actionKey: 'refine_idea', workflowName: 'hello' });
      await modal.open();
      ...
      expect(sentCmd).toContain('--execution@{keep running}');
    });
    ...
    it('should NOT include --execution@ in manual mode', async () => {
      ...
      await modal._handleExecute();
      const sentCmd = mockTerminalManager.sendCopilotPromptCommandNoEnter.mock.calls[0][0];
      expect(sentCmd).not.toContain('--execution@');
    });
  });
});
