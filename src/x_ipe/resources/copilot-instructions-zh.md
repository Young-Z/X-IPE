...
1. **仔细阅读理解** 用户的消息
2. **分类意图** — 这是修复 bug？新功能？重构？配置变更？
3. **匹配对应技能** — 扫描 `.github/skills/x-ipe-task-based-*/SKILL.md` 描述找到正确的技能
4. **加载并遵循该技能** — 技能定义了正确的流程、前置条件和完成定义
...
✅ 用户："默认端口不对，修复一下"
   Agent：*识别为 bug 修复 → 加载 x-ipe-task-based-bug-fix →
           在看板创建任务 → 写失败测试 → 修复代码 →
           验证测试通过 → 更新看板*
```
...
**如果你发现自己即将调用 `edit`、`create` 或通过 `bash` 写代码，但没有完成上述步骤 1-4 — 立即停下来。回去遵循流程。**
