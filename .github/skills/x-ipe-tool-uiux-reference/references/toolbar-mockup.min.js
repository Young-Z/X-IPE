...
window.__xipeRegisterMode('mockup', function MockupModeInit(container) {let compCounter = 0;let snapActive = false;let currentStep = 1;const MAX_COMPONENTS = 20;const SEMANTIC_TAGS = new Set(['SECTION', 'NAV', 'ARTICLE', 'ASIDE', 'HEADER', 'FOOTER', 'MAIN', 'FIGURE']);const CAPTURE_PROPS = [
...
function renderStep() {if (currentStep === 1) {snapActive = true;renderComponentList();} else if (currentStep === 2) {snapActive = false;renderInstructions();} else if (currentStep === 3) {snapActive = false;renderAnalyze();} else if (currentStep === 4) {snapActive = false;renderGenerate();}}
function renderComponentList() {const sc = stepContainers[0];if (!sc) return;sc.innerHTML = '';window.__xipeRefData.components.forEach((comp, i) => {const row = document.createElement('div');row.className = 'xipe-comp-row';row.innerHTML = `<span class="xipe-comp-tag">${comp.tag}</span><span class="xipe-comp-selector">${comp.selector}</span><span class="xipe-comp-dims">${Math.round(comp.bounding_box.width)}×${Math.round(comp.bounding_box.height)}</span>`;const rmBtn = document.createElement('button');rmBtn.className = 'xipe-comp-remove';rmBtn.textContent = '×';rmBtn.onclick = () => {window.__xipeRefData.components.splice(i, 1);if (overlays[i]) { overlays[i].remove(); overlays.splice(i, 1); }
renderComponentList();};row.appendChild(rmBtn);row.addEventListener('mouseenter', () => {if (overlays[i]) overlays[i].style.borderColor = '#f59e0b';});row.addEventListener('mouseleave', () => {if (overlays[i]) overlays[i].style.borderColor = '#10b981';});sc.appendChild(row);});if (window.__xipeRefData.components.length === 0) {sc.innerHTML = '<div style="color:#64748b;font-size:11px;text-align:center;padding:20px">Click elements on the page to select components</div>';}}
function renderInstructions() {const sc = stepContainers[1];if (!sc) return;sc.innerHTML = '';window.__xipeRefData.components.forEach((comp) => {const wrapper = document.createElement('div');wrapper.style.marginBottom = '8px';wrapper.innerHTML = `<div class="xipe-comp-row"><span class="xipe-comp-tag">${comp.tag}</span><span class="xipe-comp-selector">${comp.selector}</span></div>`;const input = document.createElement('input');input.className = 'xipe-instruction-input';input.placeholder = 'Add notes (e.g., sticky header, animated)';input.value = comp.instruction;input.addEventListener('input', (e) => { comp.instruction = e.target.value; });wrapper.appendChild(input);sc.appendChild(wrapper);});if (window.__xipeRefData.components.length === 0) {sc.innerHTML = '<div style="color:#64748b;font-size:11px;text-align:center;padding:20px">No components selected. Go back to Step 1.</div>';}}
function renderAnalyze() {const sc = stepContainers[2];if (!sc) return;sc.innerHTML = '';const summary = document.createElement('div');summary.style.cssText = 'color:#e2e8f0;font-size:11px;margin-bottom:12px';summary.textContent = `${window.__xipeRefData.components.length} components ready for analysis`;sc.appendChild(summary);const btn = document.createElement('button');btn.className = 'xipe-btn xipe-btn-primary';btn.textContent = 'Analyze';btn.disabled = window.__xipeRefData.components.length === 0;btn.onclick = () => {window.__xipeRefData.mode = 'mockup';window.__xipeRefReady = true;window.__xipeToast('Analyzing components...', 'progress');};sc.appendChild(btn);}
function renderGenerate() {const sc = stepContainers[3];if (!sc) return;sc.innerHTML = '';const summary = document.createElement('div');summary.style.cssText = 'color:#e2e8f0;font-size:11px;margin-bottom:12px';summary.textContent = `Ready to generate mockup from ${window.__xipeRefData.components.length} components`;sc.appendChild(summary);const btn = document.createElement('button');btn.className = 'xipe-btn xipe-btn-primary';btn.textContent = 'Generate Mockup';btn.disabled = window.__xipeRefData.components.length === 0;if (btn.disabled) btn.title = 'Select at least one component first.';btn.onclick = () => {window.__xipeRefData.mode = 'mockup';window.__xipeRefReady = true;window.__xipeToast('Saving reference data...', 'progress');};sc.appendChild(btn);}
buildStepUI();});})();