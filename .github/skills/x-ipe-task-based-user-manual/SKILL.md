...
| 4.1 | Decide Next Action | DAO-assisted next task decision | Next action decided |
| 4.2 | Execute Next Action | Delegate to x-ipe-workflow-task-execution sub-agent | Sub-agent started |
...
<procedure name="user-manual">
  ...
  <phase_4 name="继续执行（Continue Execute）">
    <step_4_1>
    ...
    <step_4_2>
      ...
      <action>
        Based on the decision from Step 4.1, delegate execution to a sub-agent:
        1. Invoke x-ipe-workflow-task-execution as a sub-agent (use premium model)
        2. Pass the decided next task and full context from Step 4.1
...
See [references/examples.md](.github/skills/x-ipe-task-based-user-manual/references/examples.md) for concrete execution examples.
