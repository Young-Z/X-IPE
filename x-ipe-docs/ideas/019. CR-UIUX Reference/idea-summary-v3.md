...
| View | Description |
|------|-------------|
| **Theme Mode** | Circular magnifier (120px, 10x zoom, crosshair overlay) with color swatch list. Each color shows hex value and role annotation chips (primary / secondary / accent / custom). "Create Theme" button at bottom triggers the downstream skill. |
| **Mockup Mode** | Smart-snap overlay highlighting a semantic container (`<section>.sim-hero`) with teal dashed border and corner resize handles. Component list panel with captured metadata (tag, selector, dimensions). 4-step wizard: Select → Instruct → Analyze → Generate. |
| **Magnifier** | Simulated 11×11 pixel grid inside a circular lens, following cursor position. Demonstrates the offscreen-canvas-based sampling concept. |
| **Smart Snap** | Dashed overlay positioned on a hero section with CSS `getBoundingClientRect`-based placement. Shows container tag badge and drag handles at corners. |
| **Toast Notifications** | Auto-cycling inline toasts at bottom of toolbar panel. Covers all states: info (📍), in-progress (spinner), success (✓), error (✗), and agent communication. 4s auto-dismiss. |

#### Toolbar Panel Design

- **Header**: "X-IPE Reference" with status dot + X-IPE badge
- **Mode switcher**: Two tab buttons — "Catch Theme" (palette icon) / "Copy Mockup" (layers icon)
- **Dark chrome panel** (#0f172a) with emerald accent (#10b981)
- **Fonts**: Outfit (headings) + Space Mono (code/metadata) + DM Sans (body)
- **Hamburger menu position**: Top-right of injected panel (sole entry point)

#### Demo Controls

A bottom bar with toggle buttons (not part of actual toolbar) lets you switch between:
- Theme Mode / Mockup Mode views
- Magnifier overlay toggle
- Smart Snap overlay toggle
- Toast notification cycling
