...
| `x_ipe/commands/init.py` | Project initialization handler | Creates docs/, .x-ipe/, .github/ | #init #scaffolding |
...
2. **Init:** `x-ipe init` → Scaffold creates docs/, .x-ipe/, .github/skills/
...
x-ipe init
# Output: Created docs/, .x-ipe/, .github/skills/, .x-ipe.yaml
...
def init_cmd(ctx, force: bool, dry_run: bool):
    ...
    Creates:
      - docs/ folder with ideas, planning, requirements, themes
