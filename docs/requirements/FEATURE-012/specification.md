...
Design Themes is a theming system for X-IPE that enables brands to define unified design systems (colors, typography, spacing) and apply them consistently when designing mockups for new features. Themes are stored as structured files in the `docs/themes/` folder, discoverable via a sidebar menu, and selectable via visual theme cards in the Stage Toolbox modal.
...
- [ ] AC-1.1: Themes stored in `docs/themes/` folder
...
- [ ] AC-1.5: Theme discovery scans `docs/themes/theme-*/` pattern
...
- [ ] AC-4.2: Themes menu shows folder tree of `docs/themes/`
...
**Details:**
- Input: None (scans `docs/themes/` folder)
- Process: 
  - Scan `docs/themes/theme-*/` pattern
...
- Process: Load folder tree for `docs/themes/`
...
- Icon: Palette icon (🎨)
- Behavior: Click to expand folder tree of `docs/themes/`
...
**Scenario:** `docs/themes/` folder is empty or doesn't exist.
**Expected Behavior:** Themes section shows "No themes available. Create a theme folder in docs/themes/" message.
